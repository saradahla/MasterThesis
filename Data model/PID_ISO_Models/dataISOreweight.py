...
Running my ISO algorithm for muons and outputting the score
to be used in the Z model

nohup python -u dataISOreweight.py --tag 230920_Data ../createDataset/output/MuoCutFilesForISO/230920_Data/combined_010920.h5 2>&1 &> output/logIsoReweight.txt & disown
...
parser.add_argument('--outdir', action='store', default="output/dataReweightFiles/", type=str,
...
# data["label"] = 0
# data.loc[data["muo_truthOrigin"]==13,"label"] = 1
...
reweighterEst20.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
               ...
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] >= 0.5)]]).T)
reweighterEst40.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] < 0.5)]]).T,
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
