...
nohup python -u dataPIDapply_6vars.py --tag 160920_Data_6vars  output/dataReweightFiles/110920_Data/combined_combined_train.h5 2>&1 &> output/logPidApply_6vars.txt & disown
...
print('Saving model...')
bst.save_model(args.outdir + "lgbmPID_6vars" + ".txt")
...
ax.set(xlabel = "PID prediction", ylabel = "Frequency")
ax.legend()
fig.tight_layout()
fig.savefig(args.outdir + "PidPredictionValid" + ".pdf")
#
# fig, ax = plt.subplots(1,1,figsize=(5,5))
# ax.hist(logit(y_pred_valid[y_valid == 0]), bins = 100, color = 'g', histtype="step", label = f"Background, n = {len(y_pred_valid[y_valid == 0])}")
# ax.hist(logit(y_pred_valid[y_valid == 1]), bins = 100, color = 'tab:purple', histtype="step", label = f"Signal, n = {len(y_pred_valid[y_valid == 1])}")
