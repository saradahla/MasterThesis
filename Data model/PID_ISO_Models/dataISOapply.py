...
nohup python -u dataISOapply.py --tag 230920  output/dataReweightFiles/230920_Data/combined_combined_train.h5 2>&1 &> output/logIsoApply.txt & disown
...
parser.add_argument('--outdir', action='store', default="output/ISOModels/", type=str,
...
print('Saving model...')
bst.save_model(args.outdir + "lgbmISO" + ".txt")
...
log.info(f"Plotting histogram for valid data")

fig, ax = plt.subplots(1,1,figsize=(5,5))
ax.hist(logit(y_pred_valid[y_valid == 0]), bins = 100, color = 'g', histtype="step", label = f"Background, n = {len(y_pred_valid[y_valid == 0])}")
ax.hist(logit(y_pred_valid[y_valid == 1]), bins = 100, color = 'tab:purple', histtype="step", label = f"Signal, n = {len(y_pred_valid[y_valid == 1])}")
ax.set(xlabel = "ISO prediction", ylabel = "Frequency")
ax.legend()
fig.tight_layout()
fig.savefig(args.outdir + "IsoPredictionValid" + ".pdf")

fig, ax = plt.subplots(1,1,figsize=(5,5))
ax.hist(logit(y_pred_valid[y_valid == 0]), bins = 100, color = 'g', histtype="step", label = f"Background, n = {len(y_pred_valid[y_valid == 0])}")
ax.hist(logit(y_pred_valid[y_valid == 1]), bins = 100, color = 'tab:purple', histtype="step", label = f"Signal, n = {len(y_pred_valid[y_valid == 1])}")
ax.set(xlabel = "ISO prediction", ylabel = "Frequency", ylim = (0,100))
ax.legend()
fig.tight_layout()
fig.savefig(args.outdir + "IsoPredictionValidCut" + ".pdf")
