...
data = pd.read_pickle("/groups/hep/sda/work/Zmm model/Z_model/output/ZModels/250920/train_data.pkl")
#MC = True
# data = pd.read_pickle("/Users/sda/hep/work/Zmm model/Z_model/output/ZModels/250920/train_data.pkl")
# data["muo1_truthPdgId"]

# data = pd.read_pickle("/groups/hep/sda/work/Data model/Z model/output/ZModels/230920/train_data.pkl")
# data = data[data["type"] != 1]
data = data[(data["muo1_truthPdgId"]*data["muo2_truthPdgId"]) < 0]
...
cuts = np.linspace(7.5,8.5, num = 60)
#cuts = [7.09]
# cuts = [-40]
i = 1
j = 1
...
# for x in [1]:
    ...
    # Likelihood_cut = ((data["muo1_LHTight"] == 1) & (data["muo2_LHTight"] == 1))
    BL_sig, BL_bkg, sig_ratio, bkg_ratio = PeakFit_likelihood(Likelihood_cut*1, data["invM"], "Fit Medium", plots = True, constant_mean = True,
    ...
    if BL_sig != 0:
        ...
        if (sig_ratio < nSigMedium/nSig + 0.005) & (sig_ratio > nSigMedium/nSig - 0.005):
            print(f"Found correct signal efficiency for a Medium cut at {sig_ratio} compared to true {nSigMedium/nSig}")
