...
def Plot(input, fig, ax, xlabel, includeN = False, legend = True):
    ...
    if legend:
        ax.step(x=edges_sig, y=np.append(counts_sig, 0), where="post", color = "tab:purple", alpha = 1, label = "Signal");
        ax.step(x=edges_bkg, y=np.append(counts_bkg, 0), where="post", color = "g", alpha = 1, label = "Background");
        ax.step(x=edges_bkgrw, y=np.append(counts_bkgrw, 0), where="post", color = "k", linestyle = 'dashed', alpha = 1, label = "Background reweighted");
    else:
        ax.step(x=edges_sig, y=np.append(counts_sig, 0), where="post", color = "tab:purple", alpha = 1);
        ax.step(x=edges_bkg, y=np.append(counts_bkg, 0), where="post", color = "g", alpha = 1);
        ax.step(x=edges_bkgrw, y=np.append(counts_bkgrw, 0), where="post", color = "k", linestyle = 'dashed', alpha = 1);
