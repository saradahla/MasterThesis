...
def root_plot(m_ee, distribution, fit, mZmin: int, mZmax: int, title: str):
    ...
    gPad.Modified();
    gPad.Update();
    c_mZ.Print(f'./output/figuresRooFit/20201030_Zmmg/cutsData/{title}.png') # Save file. Remember that the folder needs to exist
#
...
#cut {cutval}
    ...
    #### DATA ####
    BL_sig = 71075*(1-0.4049) # BL = baseline, the number is the fraction of bkg in baseline
    ...
    if (bkg_ratio < 1.009) & (sig_ratio < 1.009) &  (abs(mc_ZeeWenu_mZ_resid.getYAxisMin()) < 4.5) & (abs(mc_ZeeWenu_mZ_resid.getYAxisMax()) < 4.5):
        # input('....')
