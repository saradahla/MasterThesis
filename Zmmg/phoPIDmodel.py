...
nohup python -u phoPIDmodel.py --tag 20201027  output/phoReweightFiles/20201027/combined_20201027_train.h5 2>&1 &> output/logPidApply.txt & disown

nohup python -u phoPIDmodel.py --tag 20201028 --hypopt 1 output/phoReweightFiles/20201028/combined_20201028_train.h5 2>&1 &> output/logPidApply.txt & disown
...
parser.add_argument('--outdir', action='store', default="output/phoPIDModels/", type=str,
...
# if args.conv != 0:
#     data_get = data_get[data_get["pho_ConversionType"] != 0]
# else:
...
valid_dataset = lgb.Dataset(X_valid, label=y_valid, weight=data_valid["weightEst10"])
...
print('Saving model...')
bst.save_model(args.outdir + "lgbm_phoPID" + ".txt")
...
fpr_tight_train, tpr_tight_train, thresholds_tight_train = roc_curve(y_train, data_train["pho_isPhotonEMTight"], sample_weight=data_train["weightEst10"])
auc_tight_train = auc(fpr_tight_train, tpr_tight_train)

fpr_loose_train, tpr_loose_train, thresholds_loose_train = roc_curve(y_train, data_train["pho_isPhotonEMLoose"], sample_weight=data_train["weightEst10"])
auc_loose_train = auc(fpr_loose_train, tpr_loose_train)

fpr_tight_valid, tpr_tight_valid, thresholds_tight_valid = roc_curve(y_valid, data_valid["pho_isPhotonEMTight"], sample_weight=data_valid["weightEst10"])
auc_tight_valid = auc(fpr_tight_valid, tpr_tight_valid)

fpr_loose_valid, tpr_loose_valid, thresholds_loose_valid = roc_curve(y_valid, data_valid["pho_isPhotonEMLoose"], sample_weight=data_valid["weightEst10"])
auc_loose_valid = auc(fpr_loose_valid, tpr_loose_valid)

fig, ax = plt.subplots(1,1, figsize=(8,5))
for fpr, tpr, auc, name in zip(fprs, tprs, aucs, names):
    ax.plot(tpr, fpr, label="LGBM {:s} (area = {:.3f})".format(name, auc))
ax.plot(tpr_tight_train[1], fpr_tight_train[1], 'b*', label="{:s} (area = {:.3f})".format("Tight, training set", auc_tight_train))
ax.plot(tpr_loose_train[1], fpr_loose_train[1], 'bo', label="{:s} (area = {:.3f})".format("Loose, training set", auc_loose_train))

ax.plot(tpr_tight_valid[1], fpr_tight_valid[1], 'r*', label="{:s} (area = {:.3f})".format("Tight, validation set", auc_tight_valid))
ax.plot(tpr_loose_valid[1], fpr_loose_valid[1], 'ro', label="{:s} (area = {:.3f})".format("Loose, validation set", auc_loose_valid))
