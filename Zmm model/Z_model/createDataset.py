...
def signalSelection(hf, event, comb, nType):
    ...
    if (isZmm_0*isZmm_1): # Both origin from a Z
        ...
        if (isZmm_0+isZmm_1>0): # One origins from a Z
            ...
            nType[0,2] += 1
            return 3 # Bkg2Muo
        elif (muo0_isMuo+muo1_isMuo==1): # One is a muon
            nType[0,3] += 1
            return 4 # Bkg1Muo
        else: # No muons
            nType[0,4] += 1
...
def MakeFiles(arguments):
    ...
    for i, event in enumerate(np.arange(start,stop)):
        ...
        if (nMuo >= 2):
            ...
            for c in comb:
                #try:
                selection = signalSelection(hf, event, c, nType)
