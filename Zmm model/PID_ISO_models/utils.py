...
def Plot(input, fig, ax, xlabel, includeN = False, legend = True):
    ...
    if legend:
        ...
        ax.step(x=edges_bkgrw, y=np.append(counts_bkgrw, 0), where="post", color = "r", alpha = 1, label = "Background reweighted");
    else:
        ...
        ax.step(x=edges_bkgrw, y=np.append(counts_bkgrw, 0), where="post", color = "r", alpha = 1);
