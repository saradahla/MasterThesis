...
nohup python -u muoPIDreweight.py --tag 010920_ZbbW  output/PID_ISO_Dataset/010920_ZbbW/010920_ZbbW.h5 2>&1 &> output/logPidReweight.txt & disown
nohup python -u muoPIDreweight.py --tag 071020_ZbbW  output/PID_ISO_Dataset/010920_ZbbW/010920_ZbbW.h5 2>&1 &> output/logPidReweight.txt & disown
...
parser.add_argument('--outdir', action='store', default="output/PIDReweightFiles/", type=str,
...
def getWeights(datatype, reweighter, data):
    ...
    # predict the weights
    total_weight = reweighter.predict_weights(np.array([data['muo_eta'][data['label'] < 0.5],
                                                        data['muo_pt'][data['label'] < 0.5],
    ...
    weight = np.ones(len(data['muo_eta']))
...
data["label"] = 0
data.loc[np.abs(data["muo_truthPdgId"])==13,"label"] = 1
...
reweighterEst15  = GBReweighter(n_estimators=15,
                           #learning_rate=params['learning_rate'],
                           max_depth=5,
                           #min_samples_leaf=params['min_samples_leaf'],
                           #loss_regularization=params['loss_regularization']
...
reweighterEst17  = GBReweighter(n_estimators=17,
                           #learning_rate=params['learning_rate'],
                           max_depth=5,
                           #min_samples_leaf=params['min_samples_leaf'],
                           #loss_regularization=params['loss_regularization']
...
reweighterEst10.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] < 0.5)]]).T,
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] >= 0.5)]]).T)
reweighterEst20.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] < 0.5)]]).T,
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] >= 0.5)]]).T)
reweighterEst15.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] < 0.5)]]).T,
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] >= 0.5)]]).T)
reweighterEst17.fit(original = np.array([data_train['muo_eta'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] < 0.5)],
                                    data_train['correctedScaledAverageMu'][trainMask & (data_train["label"] < 0.5)]]).T,
               target   = np.array([data_train['muo_eta'][trainMask & (data_train["label"] >= 0.5)],
                                    data_train['muo_pt'][trainMask & (data_train["label"] >= 0.5)],
...
weight_trainEst15 = getWeights("train", reweighterEst15, data_train[trainMask])
weight_trainEst17 = getWeights("train", reweighterEst17, data_train[trainMask])
...
weight_validEst15 = getWeights("valid", reweighterEst15, data_train[validMask])
weight_validEst17 = getWeights("valid", reweighterEst17, data_train[validMask])

weight_testEst10  = getWeights("test",  reweighterEst10, data_test)
weight_testEst20  = getWeights("test",  reweighterEst20, data_test)
weight_testEst15  = getWeights("test",  reweighterEst15, data_test)
weight_testEst17  = getWeights("test",  reweighterEst17, data_test)

# Add weights to data
log.info(f"Add weights for training, validation and test set to data")
data_train["weightEst10"] = 0
data_train["weightEst20"] = 0
data_train["weightEst15"] = 0
data_train["weightEst17"] = 0

data_train.loc[trainMask, "weightEst10"] = weight_trainEst10
data_train.loc[trainMask, "weightEst20"] = weight_trainEst20
data_train.loc[trainMask, "weightEst15"] = weight_trainEst15
data_train.loc[trainMask, "weightEst17"] = weight_trainEst17

data_train.loc[validMask, "weightEst10"] = weight_validEst10
data_train.loc[validMask, "weightEst20"] = weight_validEst20
data_train.loc[validMask, "weightEst15"] = weight_validEst15
data_train.loc[validMask, "weightEst17"] = weight_validEst17

data_test["weightEst10"] = weight_testEst10
data_test["weightEst20"] = weight_testEst20
data_test["weightEst15"] = weight_testEst15
data_test["weightEst17"] = weight_testEst17
...
#============================================================================
weights_train = [weight_trainEst10, weight_trainEst20, weight_trainEst15, weight_trainEst17]
weights_train_names = ["weight_trainEst10", "weight_trainEst20", "weight_trainEst15", "weight_trainEst17"]

### Training data
for w, wn in zip(weights_train, weights_train_names):
    fig, ax = plt.subplots(2,2,figsize=(15,10))
    ax = ax.flatten()
    fig, ax[0] = Plot(Histogram(data_train['muo_eta'][trainMask], data_train["label"][trainMask], w, 120, -4, 4), fig, ax[0], r"$\eta$", includeN = True)
    fig, ax[1] = Plot(Histogram(data_train['muo_pt'][trainMask], data_train["label"][trainMask], w, 120, -5, 120000), fig, ax[1], "pt", includeN = False)
    fig, ax[2] = Plot(Histogram(data_train['correctedScaledAverageMu'][trainMask], data_train["label"][trainMask], w, 80, -2, 80), fig, ax[2], r"$\langle\mu\rangle$", includeN = False)
...
### Validation data
weights_valid = [weight_validEst10, weight_validEst20, weight_validEst15, weight_validEst17]
weights_valid_names = ["weight_validEst10", "weight_validEst20", "weight_validEst15", "weight_validEst17"]

for w, wn in zip(weights_valid, weights_valid_names):
    fig, ax = plt.subplots(2,2,figsize=(15,10))
    ax = ax.flatten()
    fig, ax[0] = Plot(Histogram(data_train['muo_eta'][validMask], data_train["label"][validMask], w, 120, -4, 4), fig, ax[0], r"$\eta$", includeN = True)
    fig, ax[1] = Plot(Histogram(data_train['muo_pt'][validMask], data_train["label"][validMask], w, 120, -5, 120000), fig, ax[1], "pt", includeN = False)
    fig, ax[2] = Plot(Histogram(data_train['correctedScaledAverageMu'][validMask], data_train["label"][validMask], w, 80, -2, 80), fig, ax[2], r"$\langle\mu\rangle$", includeN = False)
